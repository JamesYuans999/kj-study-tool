...
def save_ai_pref():
    ...
    m = None
    if "OpenRouter" in str(p):
        m = st.session_state.get('or_model_select')
    elif "DeepSeek" in str(p):
        m = st.session_state.get('ds_model_select')
    elif "Gemini" in str(p):
        m = st.session_state.get('gl_model_select')
    elif "Glama" in str(p):
        m = st.session_state.get('glama_model_id')
...
def fetch_google_models(api_key):
    try:
        ...
        return [m['name'].replace("models/", "") for m in data.get('models', []) if
...
def get_cached_outline_v2(chapter_id, text_content, uid):
    """
    [V15.0 防压缩版] 全文扫描大纲生成
    核心修复：解决 AI 将数百个考点归纳为几个章节标题的问题。
    新增策略：如果 AI 汇总后数量骤减，强制使用 Map 阶段的原始列表。
    """
    import json
    import math

    # 1. 优先查库 (如果数据库里有且数量足够多，直接用)
    try:
        db_res = supabase.table("chapters").select("outline").eq("id", chapter_id).execute()
        if db_res.data and db_res.data[0].get('outline'):
            cached_outline = db_res.data[0]['outline']
            # 如果缓存里已经有超过 20 个点，说明质量不错，直接返回
            if len(cached_outline) > 20:
                print("🚀 Hit DB Outline Cache (Good Quality)!")
    ...
    domain_knowledge = """
    【关键指令：拒绝目录，只要干货】
    1. **禁止**提取“第一章 总论”、“第二节 公司法”这种大标题。
    2. **必须**提取具体的法条、规则或考点。
       - ❌ 错误：诉讼时效
       - ✅ 正确：诉讼时效的中止与中断区别
       - ✅ 正确：普通诉讼时效的3年规定
    3. 目标是生成一份“背诵清单”，而不是“书籍目录”。
    """

    # A. 语义分块
    chunks = semantic_chunking(text_content, max_chunk_size=10000)  # 稍微切小一点，提高精度
    all_sub_points = []
    ...
    for i, chunk in enumerate(chunks):
        progress_text.caption(f"🤖 AI 正在挖掘第 {i + 1}/{len(chunks)} 部分的考点（清单模式）...")

        map_prompt = f"""
        【任务】从这段教材中提取所有具体的“必背法条”或“核心考点”。
        {domain_knowledge}
        【片段内容】...{chunk[:12000]}...
        【要求】
        1. 尽可能多地提取，**每个片段至少提取 15 个点**。
        2. 如果遇到列举项（1,2,3...），请拆分成独立的知识点。
        3. 仅返回 JSON 字符串数组。
        """
        try:
            ...
            if isinstance(chunk_res, list):
                # 简单的清洗：去掉过短的条目（如“前言”）
                valid_items = [str(x).strip() for x in chunk_res if len(str(x)) > 4]
                all_sub_points.extend(valid_items)
        except:
            continue
    ...
    if not all_sub_points:
        return ["本章概览", "核心考点", "章节总结"]

    # 🟢 核心策略：数量检查
    # 如果 Map 阶段已经提取了 50 个点，我们就不让 AI 做深度汇总了，
    # 因为 AI 倾向于把 50 个点归纳成 5 个点。我们只让它做“去重”。

    total_extracted = len(all_sub_points)
    final_outline = []

    if total_extracted > 40:
        # 策略 A：直接使用 Python 去重，跳过 AI 汇总（防止变少）
        # 这种方式最安全，能保住那 120 个法条
        seen = set()
        final_outline = [x for x in all_sub_points if not (x in seen or seen.add(x))]
        print(f"🔥 Skip AI Reduce to preserve quantity: {len(final_outline)} items")
    else:
        # 策略 B：数量不多，让 AI 整理一下顺序
        reduce_prompt = f"""
        【任务】整理以下会计考点列表。
        {domain_knowledge}
        【原始列表】{json.dumps(all_sub_points, ensure_ascii=False)}
        【要求】
        1. 去除完全重复的项。
        2. 按逻辑顺序排序。
        3. **严禁合并**：保持列表的长度，不要把多个点合并成一个标题。
        4. 返回纯 JSON 字符串数组。
        """
        try:
            ai_res = call_ai_json(reduce_prompt)
            if isinstance(ai_res, list) and len(ai_res) > 5:
                final_outline = ai_res
            else:
                final_outline = all_sub_points  # AI 把它压缩没了，回滚到原始列表
        except:
            final_outline = all_sub_points

    # 双重保险：如果最终结果还是被压缩得太厉害（少于原始的 30%），强制回滚
    if len(final_outline) < total_extracted * 0.3 and total_extracted > 10:
        seen = set()
        final_outline = [x for x in all_sub_points if not (x in seen or seen.add(x))]
...
with st.sidebar:
    ...
    if "Gemini" in prov:
        ...
        st.session_state.google_model_id = st.selectbox("🔌 模型", opts, index=idx_m, key="gl_model_select",
    ...
    # 2. DeepSeek
        ...
        idx_m = opts.index(saved_m) if saved_m in opts else 0
        st.session_state.deepseek_model_id = st.selectbox("🔌 模型", opts, index=idx_m, key="ds_model_select",
                                                          on_change=save_ai_pref)
    ...
    # 3. OpenRouter
        ...
        try:
            ...
            st.session_state.openrouter_model_id = st.selectbox("🔌 模型", final_ids, index=idx_m, key="or_model_select",
                                                                on_change=save_ai_pref)
        except:
            st.error("OpenRouter 连接失败")
...
if menu == "🏠 仪表盘":
    ...
    with c3:
        # 简单计算错题数
        try:
            err_count = supabase.table("user_answers").select("id", count="exact").eq("user_id", user_id).eq(
                "is_correct", False).execute().count
        except:
            err_count = 0
        st.markdown(f"""
        <div class="css-card">
        ...
        # -------------------------------------------------
            ...
            # >>>>>>>>> 分支 B: 习题库导入 (新增逻辑) <<<<<<<<<
                ...
                q_template = [
                    ...
                    {
                        "章节名称": "第二章 存货",
                        "题型(single/multi/judgment/subjective)": "multi",
                        "题目内容": "下列属于存货的有...",
    ...
    with tab_view:
        ...
        with tab_gen:
            ...
            if not mats:
                ...
                # --- 核心状态 Key 定义 ---
                ACTIVE_ID_KEY = f"active_lesson_id_{cid}_{user_id}"
                ...
                # 🟢 V13新增：大纲覆盖状态的手动覆写记录 {标题: True/False}
                OUTLINE_OVERRIDES_KEY = f"outline_overrides_{cid}_{user_id}"

                # --- 初始化 Session ---
                if ACTIVE_ID_KEY not in st.session_state: st.session_state[ACTIVE_ID_KEY] = "new"
                ...
                if BACKUP_DRAFT_KEY not in st.session_state: st.session_state[BACKUP_DRAFT_KEY] = None
                if OUTLINE_OVERRIDES_KEY not in st.session_state: st.session_state[OUTLINE_OVERRIDES_KEY] = {}
                ...
                if not st.session_state[OUTLINE_KEY]:
                    ...
                    with st.expander("🗺️ 大纲管理", expanded=False):
                        if st.button("🔥 销毁大纲并重置"):
                            st.session_state[OUTLINE_KEY] = []
                            st.session_state[OUTLINE_OVERRIDES_KEY] = {}  # 清空手动状态
                ...
                # 1. 自动检测 (V13.1 增强版函数)
                raw_status = check_outline_coverage_v2(outline_data, current_draft)

                # 2. 融合手动覆写状态
                overrides = st.session_state[OUTLINE_OVERRIDES_KEY]
                final_status = []
                for item in raw_status:
                    title = item['title']
                    # 逻辑：如果 overrides 里有记录，优先用记录；否则用自动检测结果
                    is_covered = overrides.get(title, item['covered'])
                    final_status.append({"title": title, "covered": is_covered})
                ...
                with c_p2:
                    if final_status:
                        covered_cnt = sum(1 for x in final_status if x['covered'])
                        st.metric("🗺️ 知识点覆盖", f"{covered_cnt}/{len(final_status)}")
                        st.progress(covered_cnt / len(final_status) if len(final_status) > 0 else 0)
                    else:
                        st.metric("🗺️ 知识点", "--")
                with c_p3:
                    CHUNK_SIZE = 3500
                    ...
                    if final_status:
                        for idx, item in enumerate(final_status):
                            ...
                            with c_btn:
                                ...
                                # 利用 callback 避免直接 rerun 导致的闪烁（可选），这里用 key 区分
                                if st.button(btn_label, key=f"tg_{idx}_{cid}", help="点击手动切换状态 (强行变绿/红)"):
                        ...
                        if missing_items:
                            if st.button("⚡ 一键补全红圈", type="primary", help="AI 自动查漏补缺"):
                                ...
                                try:
                                    ...
                                    st.session_state[CURSOR_KEY] = total_len  # 视为读完

                                    # 🟢 自动保存逻辑 (适配多版本)
                                    active_id = st.session_state[ACTIVE_ID_KEY]
                                    # 获取当前标题框的值(需要从下面的控件取，或者用默认值)
                                    curr_title = st.session_state.get(f"title_val_{cid}", f"深度解析：{c_name}")

                                    upsert_data = {
                                        "user_id": user_id, "chapter_id": cid, "title": curr_title,
                                    ...
                                    if active_id and active_id != "new":
                                        ...
                                        st.session_state[ACTIVE_ID_KEY] = new_res.data[0]['id']
                        ...
                        if st.button("🧹 重置本章进度"):
                            ...
                            st.session_state[OUTLINE_OVERRIDES_KEY] = {}  # 重置手动状态
                            st.rerun()
                    else:
                        st.caption("暂无大纲")
                ...
                # >>> 右侧：多版本管理 + 预览 + 生成 <<<
                with col_right:
                    # === 🟢 V12: 多版本存档选择器 (之前丢失的部分) ===
                    try:
                        history = supabase.table("ai_lessons").select(
                            "id, title, updated_at, content, current_cursor, ai_model").eq("chapter_id", cid).eq(
                            "user_id", user_id).order("updated_at", desc=True).execute().data
                    except:
                        history = []

                    lesson_opts = ["➕ 新建空白讲义"] + [f"📄 {l['title']} ({l['updated_at'][5:10]} 更新)" for l in
                                                        history]
                    def_idx = 0
                    curr_aid = st.session_state.get(ACTIVE_ID_KEY)

                    # 自动选中逻辑
                    if (curr_aid == "new" or curr_aid is None) and history:
                        def_idx = 1
                        latest = history[0]
                        st.session_state[ACTIVE_ID_KEY] = latest['id']
                        st.session_state[DRAFT_KEY] = latest['content']
                        st.session_state[EDITOR_KEY] = latest['content']
                        st.session_state[CURSOR_KEY] = latest.get('current_cursor') or 0
                        curr_aid = latest['id']
                    elif curr_aid != "new":
                        ids = ["new"] + [l['id'] for l in history]
                        if curr_aid in ids: def_idx = ids.index(curr_aid)

                    sel_label = st.selectbox("📂 选择要续写的存档 (默认显示最新)", options=lesson_opts, index=def_idx,
                                             key=f"lesson_sel_{cid}")

                    # 监听切换
                    target_id = "new"
                    if "➕" not in sel_label:
                        sel_idx = lesson_opts.index(sel_label)
                        target_l = history[sel_idx - 1]
                        target_id = target_l['id']

                    if target_id != st.session_state[ACTIVE_ID_KEY]:
                        st.session_state[ACTIVE_ID_KEY] = target_id
                        if target_id == "new":
                            st.session_state[DRAFT_KEY] = "";
                            st.session_state[EDITOR_KEY] = "";
                            st.session_state[CURSOR_KEY] = 0
                        else:
                            st.session_state[DRAFT_KEY] = target_l['content']
                            st.session_state[EDITOR_KEY] = target_l['content']
                            st.session_state[CURSOR_KEY] = target_l.get('current_cursor') or 0
                        st.rerun()
                    ...
                    with c_conf2:
                        # 动态标题处理
                        default_title = f"深度解析：{c_name}"
                        if st.session_state[ACTIVE_ID_KEY] != "new":
                            curr_l = next((l for l in history if l['id'] == st.session_state[ACTIVE_ID_KEY]), None)
                            if curr_l: default_title = curr_l['title']
                        lesson_title = st.text_input("标题", value=default_title, key=f"title_val_{cid}",
                                                     label_visibility="collapsed")
                    ...
                    with st.container(border=True):
                        if is_editing:
                            ...
                            if st.session_state[DRAFT_KEY]:
                                ...
                                st.info("👋 准备就绪！请点击下方按钮开始生成。")

                    # --- 底部控制栏 ---
                    start_idx = st.session_state[CURSOR_KEY]
                    end_idx = min(start_idx + CHUNK_SIZE, total_len)
                    # 完成判断：物理进度走完 OR (红圈全绿)
                    is_finished = (start_idx >= total_len) or (final_status and all(x['covered'] for x in final_status))

                    st.divider()

                    # === 🟢 V13 复活功能：进度微调 (指定章节生成) ===
                    # 只有未完成时才需要调整起点
                    if not is_finished:
                        with st.expander("📍 调整生成起点 (跳过前面/指定章节)", expanded=False):
                            c_adj1, c_adj2 = st.columns([3, 1])
                            with c_adj1:
                                # 用 Slider 调整百分比
                                current_pct = int(min(start_idx / total_len, 1.0) * 100) if total_len > 0 else 0
                                new_prog = st.slider("拖动滑块调整进度", 0, 100, current_pct, format="%d%%")
                            with c_adj2:
                                if st.button("📍 设定进度"):
                                    new_idx = int((new_prog / 100) * total_len)
                                    st.session_state[CURSOR_KEY] = new_idx
                                    st.toast(f"进度已跳转至 {new_prog}%")
                                    time.sleep(0.5)
                                    st.rerun()
                    ...
                    with b_col1:
                        if is_editing:
                            ...
                            if st.button("🎓 生成结语 (Auto-Save)", type="primary", use_container_width=True):
                                # (结语逻辑略，与之前一致，记得存 current_cursor)
                                pass
                        else:
                            ...
                            with gen_col:
                                ...
                                if st.button(btn_txt, type="primary", use_container_width=True):
                                    ...
                                    try:
                                        ...
                                        # 🟢 V13 Prompt 注入：把没变绿的标题塞给 AI
                                        pending_titles = [x['title'] for x in final_status if not x['covered']][:5]
                                        outline_hint = f"**重点讲解以下知识点**：{', '.join(pending_titles)}" if pending_titles else ""
                                        ...
                                        with st.spinner("AI 备课中..."):
                                            ...
                                            if res:
                                                ...
                                                upsert_data = {
                                                    ...
                                                    "current_cursor": next_pos, "updated_at": "now()"
                                                }
                                                active_id = st.session_state[ACTIVE_ID_KEY]
                            ...
                            with undo_col:
                                if st.session_state[BACKUP_DRAFT_KEY]:
                                    if st.button("↩️ 撤销", use_container_width=True):
                                        ...
                                        if active_id and active_id != "new":
                                            supabase.table("ai_lessons").update({
                                                "content": st.session_state[DRAFT_KEY],
                                                "current_cursor": st.session_state[CURSOR_KEY]
...
# =========================================================
        ...
        if subjects:
            ...
            if cid:
                ...
                try:
                    ...
                    done_ids = []
                    if total_q > 0:
                        user_correct = supabase.table("user_answers").select("question_id").eq("user_id", user_id).eq(
                            "is_correct", True).execute().data
    ...
    if st.session_state.get('quiz_active'):
        ...
        # 边界保护
        if idx < 0: idx = 0
        if idx >= data_len: idx = data_len - 1
        ...
        # --- 提交按钮 ---
        if st.button("✅ 提交答案", use_container_width=True, key=f"btn_sub_{idx}") and not st.session_state[sub_key]:
            if not user_val:
                st.toast("⚠️ 请先作答再提交")
            else:
...
# === 📊 弱项分析 ===
    ...
    try:
        ...
        if rows:
            ...
            c1, c2 = st.columns(2)
            with c1:
                st.markdown(f"<div class='css-card'>总刷题<div class='stat-value'>{len(df)}</div></div>",
                            unsafe_allow_html=True)
            with c2:
                ok = len(df[df['is_correct'] == True])
                st.markdown(
                    f"<div class='css-card'>正确率<div class='stat-value'>{int(ok / len(df) * 100)}%</div></div>",
                    unsafe_allow_html=True)
...
# =========================================================
    ...
    if profile.get('exam_date'):
        try:
            curr_date = datetime.datetime.strptime(profile['exam_date'], '%Y-%m-%d').date()
        except:
            pass
