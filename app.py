...
# =========================================================
    ...
    # --- 场景 B: 已有书籍管理 (保持不变) ---
        ...
        with c_tit: st.markdown(f"### 📘 {sel_book}")
        with c_act:
            if st.button("🗑️ 删除本书"):
        ...
        if not chapters: st.info("暂无章节")
            ...
            for chap in chapters:
                ...
                with st.expander(f"📑 {chap['title']} (题:{q_cnt})"):
                    if m_cnt: st.caption("教材已录入")
                    if st.button("🗑️ 清空", key=f"c_{chap['id']}"):
